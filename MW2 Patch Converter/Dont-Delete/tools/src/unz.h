...
int rnc_unpack(const unsigned char* input, unsigned char* output, unsigned long have_ret_len);
...
int lzo_compress(u8 *in, int insz, u8 *out, int outsz, int type) {
    ...
    if(!wrkmem) wrkmem = malloc(LZO1_MEM_COMPRESS);

    len = outsz;
    err = lzo1_compress(in, insz, out, &len, wrkmem);
    if(err != LZO_E_OK) {
...
int unxmemlzx(u8 *in, int insz, u8 *out, int outsz) {
    ...
    if(!ctx) {
        if(comtype_dictionary) {
            ...
            get_parameter_numbers(comtype_dictionary, 2,
                (int *)&param->WindowSize, (int *)&param->CompressionPartitionSize);
        }
...
int xmem_compress(u8 *in, int insz, u8 *out, int outsz) {
    ...
    ret = outsz;
    hr = XMemCompress(ctx, out, &ret, in, insz + MYALLOC_ZEROES);
    // + MYALLOC_ZEROES: ehmmmm long story, watch myalloc() and DMC4
...
int ungzip(u8 *in, int insz, u8 **ret_out, int *full_outsz) {
    ...
    if(in[1] == 0x8b) {         // gzip
        ...
        return(gz_unpack(in + 2, insz - 2, *ret_out, *full_outsz));
    } else if(in[1] == 0x9d) {  // lzw (experimental with known size only)
        return(uncompress_lzw(in + 3, insz - 3, *ret_out, *full_outsz, in[2]));
    } else if(in[1] == 0xa0) {  // lzh (experimental with known size only)
        return(unlzh(in + 2, insz - 2, *ret_out, *full_outsz));
    } else return(-1);
    inl = in + insz;
...
int another_rle(u8 *in, int insz, u8 *out, int outsz) {
    ...
    if(comtype_dictionary) {
        ...
        get_parameter_numbers(comtype_dictionary, 1, &escape_chr);
    }
...
int unlzrw3(unsigned char *in, int insz, unsigned char *out, int outsz) {
    ...
    if(!wrk_mem) wrk_mem = malloc(4096);
